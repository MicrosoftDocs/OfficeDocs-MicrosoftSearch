...
A search vertical can surface results from multiple connector sources. This provides flexibility in designing your search result page. The vertical setup experience allows admins to select multiple connections in the "Content Source" step.

If you accurately appoint as many semantic labels as possible, this experience is enhanced. You can add semantic labels at the point of schema definition and ingestion. See [additional information about how to create and manage semantic labels](configure-connector.md#step-5-assign-property-labels).

> [!NOTE]
> Multiple connections in a vertical is currently in preview. For more information, see [Connectors preview features](connectors-overview.md#what-are-the-preview-features).

### Things you should know

- A connection can be added as a content source only under one vertical. You can't use connections under multiple verticals.
- If you need to set up a query for a search vertical where multiple connection sources have been added, use common source properties to create the query.
...
You can define how results are displayed in the vertical by using result types. The result layout lets you show important information directly in the search results, so users don't have to select each result to see if they found what they're looking for.
...
You can create your own search result layout and override the default search result layout by creating a *result type*. A search result type is a rule that causes distinct kinds of search results to be displayed in different ways. It consists of the following:
